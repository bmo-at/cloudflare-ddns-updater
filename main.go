...
func (c *CloudflareDDNSUpdaterApplication) configure() {
	...
	if api_token, exists := os.LookupEnv(API_TOKEN_ENV_VARIABLE_NAME); exists {
		...
		c.logger.Errorf("no API token found in env var '%s'", API_TOKEN_ENV_VARIABLE_NAME)
		c.exit()
	}
	...
	if zone_name, exists := os.LookupEnv(ZONE_ENV_VARIABLE_NAME); exists {
		...
		c.logger.Errorf("no zone name found in env var '%s'", ZONE_ENV_VARIABLE_NAME)
		c.exit()
	}
	...
	if record_name, exists := os.LookupEnv("CLOUDFLARE_RECORD_NAME"); exists {
		...
		c.logger.Errorf("no record name found in env var '%s'", RECORD_ENV_VARIABLE_NAME)
		c.exit()
	}
...
func (c *CloudflareDDNSUpdaterApplication) initialize() {
	...
	if err != nil {
		c.logger.Errorf("could not create cloudflare api client with the provided token, %s", err.Error())
		c.exit()
	}
	c.api = api
	...
	if err != nil {
		c.logger.Errorf("current ip info endpoint '%s' could not be requested: %s", c.ip_info_url, err.Error())
	}
...
func (c *CloudflareDDNSUpdaterApplication) update(ctx context.Context) {
	...
	if err != nil {
		c.logger.Errorf("error when requesting the current ip from '%s': %s", c.ip_info_url, err.Error())
		c.exit()
	}
	...
	if err != nil {
		c.logger.Errorf("error reading the body of the ip request response: %s", err.Error())
		c.exit()
	}
	...
	if current_ip == nil {
		c.logger.Errorf("current IP address could not be parsed from '%s'", string(ip_bytes))
		c.exit()
	}
	...
	if err != nil {
		c.logger.Errorf("could not list zones: %s", err.Error())
		c.exit()
	}

	if len(zones) < 1 {
		c.logger.Errorf("no zones found for '%s'", c.zone_name)
		c.exit()
	}
	...
	if err != nil {
		c.logger.Errorf("could not list records for '%s': %s", c.record_name, err.Error())
		c.exit()
	}

	if len(records) < 1 {
		c.logger.Errorf("no A records found for '%s'", c.record_name)
		c.exit()
	}
	...
	if records[len(records)-1].Content != current_ip.String() {
		...
		if err != nil {
			c.logger.Errorf("could not update record '%s' in zone '%s': %s", c.record_name, c.zone_name, err)
			c.exit()
		}
	}
